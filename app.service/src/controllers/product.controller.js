  ...
  products = async (req, res) => {
    //await Authorization.verify(req, res).then(async ({companyBusinessId, companyId, userId}) => {
      try {
        ...
        const products = await db.Produto.findAll({attributes: ['codprod', 'descricao', 'un', 'emb', 'custo']})
        
        const prices = await db.ProdutoPreco.findAll({attributes: ['transacao', 'numtab', 'codprod', 'preco']})
